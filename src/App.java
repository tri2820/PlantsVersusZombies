...
public class App {
    ...
    public static void main(String[] args) {
        ...
        while (game.entities().stream().map(entity -> entity.getSingleComponent(Health.class).getHealth()).mapToInt(Integer::intValue).sum() > 0) {
            System.out.println(game.entities().stream().map(entity -> entity.getSingleComponent(Health.class).getHealth()).collect(Collectors.toList()));
            game.step();
        }
